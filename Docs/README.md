...
| [DEPENDENCY_INJECTION.md](./DEPENDENCY_INJECTION.md) | 闭包/工厂注入；可选 FactoryKit；**可注册域 ⊆ `Sendable`** |
...
| [DEFAULT_ACTOR_ISOLATION_AND_REDUX.md](./DEFAULT_ACTOR_ISOLATION_AND_REDUX.md) | MainActor 默认隔离与领域模块；对 DI 注册的影响 |
...
Demo 对照：`Examples/TGReduxKitDemo`（唯一 Composition Root `DemoRootView` → 各 Middleware 工厂参数）。
